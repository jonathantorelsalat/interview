...
public interface WishDao extends JpaRepository<Wish, Integer> {
	...
	@Query(value = "SELECT wis.status as wishStatus, com.name as nomCompanyVolontaire, pro.id as volontaireId,"
			...
			+ " ini.title as nomMission, ini.street_name as streetName, ini.city as city, ini.postal_code as postalCode, ini.country as country,"
			...
			+ " WHERE wis.status IN (:statusList)", nativeQuery = true)
